...
var EditDataDialog = function(ui, cell)
{
	...
	var attrCellType = cell.getAttribute('type');
	...
	var form = new mxForm('properties' + (attrCellType ? ' ' + attrCellType : ''));
	...
	{
		if (attrs[i].nodeName != 'label' && attrs[i].nodeName != 'type' && attrs[i].nodeName != 'placeholders')
		{
	...
	top.style.cssText = 'overflow-y:auto;overflow-x:hidden;position:absolute;left:30px;right:30px;top:30px;bottom:80px;';
...
var LinkDialog = function(editorUi, initialValue, btnLabel, fn)
{
	...
	linkInput.setAttribute('placeholder', 'http://www.energybuilder.com/');
