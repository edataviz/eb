...
(function ($) {
  ...
  WCP.ColorPicker.createWidget = function() {
    ...
    var $widget = $(
      "<div class='jQWCP-wWidget'><button class='btn-clear-color' title='Clear color'>X</button>" + 
	...
	var self = this;
	$widget.find('.btn-clear-color').on('click', function(){
		//$widget.data('jQWCP.instance').hide();
		//$($(this).parent().data('jQWCP.instance').input).blur();
		WCP.Handler.input_blur_popup($(this).parent().data('jQWCP.instance'));
	});
  ...
  WCP.ColorPicker.prototype.show = function() {
	...
	var css = {}, bound = input.getBoundingClientRect();
	var left = bound.left - WCP.ORIGIN.left;
	if(left + $widget.width() > window.innerWidth)
		css.right = window.innerWidth - bound.right;
	else
		css.left = left;
	var top = bound.top - WCP.ORIGIN.top + $input.outerHeight();
	if(top + $widget.height() > window.cHeight){
		css.bottom = window.innerHeight - bound.top;
	}
	else
		css.top = top;
    $widget.css(css);
  ...
  WCP.Handler.input_change = function( e ) {
	...
	if(color === false){
		$(instance.input).css('background', 'transparent');
	}
