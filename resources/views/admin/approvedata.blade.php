...
{{--<input type="button" value="Load Data Table" id="buttonAdd" name="buttonAdd" onClick="_approve.loadData()" style="width: 150px; height: 30px;foat:left;">--}}
<div style="float: left;">
	<div>
		<input type="button" value="Approve Data" id="buttonAppData" name="buttonAdd" onClick="_approve.approveAll('A')" style="width: 150px;">
		<input type="button" value="Set back to Validate" id="buttonSetValidate" name="buttonAdd" onClick="_approve.approveAll('V')" style="width: 170px;">
	</div>
	<div style="margin-top: 3px;">
		<input type="button" value="Check Status" id="buttonAdd" name="buttonCheckStatus" onClick="_approve.loadChart()" style="width: 150px;background-color: green;">
		<input type="button" value="Set back to Provisional" id="buttonSetPro" name="buttonAdd" onClick="_approve.approveAll('P')" style="width: 170px;">
	</div>
</div>
@stop
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("ul.multiselect-container").css({"overflow":"scroll","width":"270px","height":height-197,"margin":"-13px 0 0 5px","z-index":"0"});
		...
		$("select#DataTableGroup").attr("onchange","_approve.loadData()");
...
//			$('#bodyList').html(str);
		...
		approveAll : function(prefix){
			...
			var act = (prefix=='V'?'set back to Validate':(prefix=='P'?'set back to Provisional':'approve'));
			if(table==null)
			{
            ...
            var param = (true);
			var dateFrom = $('#date_begin').val();//.replace('-', '/');
			var dateTo = $('#date_end').val();//.replace('-', '/');
			var param2 = {
					'FACILITY_ID' : $('#Facility').val(),
					'DATE_FROM' : dateFrom,
					'DATE_TO' : dateTo,
			...
			sendAjax('/am/approveData', param, function(data){
				alert(data.message);
				//if(typeof data.result == "object" && data.result !=null) _approve.listData(data.result);
			});
		},
}
</script>
	...
	<table style="table-layout: fixed; width: 100%;">
		<thead id="table5">
			<tr>
				...
				{{--<td style="float: right; margin: 5px 7px 0 0;"><input style="margin: 0px" class="chckAll" type="checkbox" id="checkAll">Check all</td>--}}
...
<div id="CheckStatus" style="float: left;padding-top: 23px;">
	{{--check status--}}
</div>
...
{{--<input type="button" value="Set back to Provisional" onclick="_approve.approveAll('P')" style="width:230px;margin-top:5px">--}}
{{--</div>--}}
@stop
