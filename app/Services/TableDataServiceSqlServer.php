...
// version 2014-03-11
class TableDataServiceSqlServer{
    ...
    */
    ...
    function setModelName($tablename){
		$sql = "select columnproperty(object_id('$tablename'),'ID','IsIdentity')";
		$tmp = $this->query($sql);
		$this->autoID = ($tmp[0][''] == '1');
    ...
    function sql_insert(){
        ...
        foreach($_POST as $key => $val){
            if(strtoupper($key) == "ID" && $this->autoID) continue;
		...
		//$this ->query("SET IDENTITY_INSERT [$this->table] ON");
        $identity_id = $this->query_insert($sql, $sql_param, 'insert()');
		//$this ->query("SET IDENTITY_INSERT [$this->table] OFF");
...
//                $sql_final = "update CODE_ALLOC_TYPE set CODE = :CODE, NAME = :NAME, ACTIVE = :ACTIVE, [ORDER] = :ORDER where id = :identity_id";
...
//echo "identity_name:$this->identity_name / get:".$_GET[$this->identity_name];
		...
		$this->form_identity_value = $identity_id;
//echo "a:".$_GET[$this->identity_name];
        ...
        // loop thru fields
        foreach($columns as $column_name){
            if($column_name == $this->identity_name && ($this->form_display_identity == false || $action == 'add') && $this->autoID)
                continue;
            ...
            if($column_name == $this->identity_name && $this->autoID)
...
//        $this->identity_name = "ID";
        ...
        if($_pagination_off == 0 && $_export == 0)
			$sql = "SELECT a.* FROM 
(SELECT *, ROW_NUMBER() OVER (ORDER BY $s_order) as row FROM ($sql) x) a
WHERE a.row > $_offset and a.row <= ".($grid_limit+$_offset);
...
return true;
    ...
    function get_input_control($column_name, $value, $command, $called_from){
		...
		if(strtoupper($column_name)=='ID' && $this->form_identity_value>0)
    ...
    function get_columns($context = ''){
		...
		*/
        ...
        $sql = "select column_name, column_default from information_schema.columns where column_default is not null and table_name = :table AND TABLE_SCHEMA='eb4sqlserver'";
    ...
    function get_pagination($count, $limit, $_offset, $_pagination_off){
        ...
        if($count > 100000){
            $input = "<input type='text' size='7' class='active_page_$id' id='active_page_$id' value='$active_page' /><input type='button' value='$this->pagination_text_go' onclick='window.location.href=\"{$uri_path}_offset=\" + ((document.getElementById(\"active_page_$id\").value * $limit) - $limit) + \"&amp;$get\".replace(/&amp;/, \"&\")' />";
        }
        else
        {
