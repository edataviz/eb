...
class EventsEmissionSourcesController extends CodeController
{
...
//                                            'model'		=>'EmissionFactorTable']
    ...
    public function getDataSet($postData, $dcTable, $facility_id, $occur_date, $properties)
    {
        ...
        if ($mdlName == "EventEmissionGroup") {
            ...
            $dataSet = $mdl::whereDate("$dcTable.EFFECTIVE_DATE", '<=', $date_end)
        ...
        } else if ($mdlName == "EventEmissionCalcMethod") {
            $columns = $this->extractRespondColumns($dcTable,$properties);
            if (!$columns) $columns = [];
            array_push(
                $columns,
                "$dcTable.EVENT_EMISSION_GROUP_ID",
                "$dcTable.ID",
                "$dcTable.ID as DT_RowId",
                "$dcTable.EVENT_EMISSION_GROUP_ID as $dcTable"
            );
            $dataSet = $mdl::where("$dcTable.EVENT_EMISSION_GROUP_ID",'=',$postData['id'])
                ->select($columns)
                ->get();
